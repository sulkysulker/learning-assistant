...
def _build_set_summary(flashcard_set: FlashcardSet, cards_count: int) -> dict:
	return {
...
def _build_set_overview(flashcard_set: FlashcardSet, document_name: str, cards_count: int) -> dict:
	return {
		"id": str(flashcard_set.id),
...
def list_flashcard_sets(db: Session, current_user: User, document_id: str) -> dict:
	...
	cards_counts_map = dict(cards_counts)

	return {
		"flashcard_sets": [
			_build_set_summary(flashcard_set, cards_counts_map.get(flashcard_set.id, 0))
...
def list_user_flashcard_sets(db: Session, current_user: User) -> dict:
	rows = (
		...
		.join(Document, Document.id == FlashcardSet.document_id)
	...
	return {
		"flashcard_sets": [
			_build_set_overview(
				flashcard_set=row[0],
				document_name=row[1],
				cards_count=cards_counts_map.get(row[0].id, 0),
			)
			for row in rows
		]
	}
